...
class StartPersonalForm extends Component {
  ...
  retrieveValues() {
    const values = this.child.retrieveValues();
    const send = STATES[1].states.length - 1 === this.props.index;
    if (this.props.index !== null && this.props.index !== undefined) {
      return { key: STATES[1].states[this.props.index].key, value: values, send };
    } return values;
  }
  ...
  render() {
    ...
    const question = STATES[1].states[index];
    let answer;
