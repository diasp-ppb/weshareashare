const images = {
  logo: require('@assets/weshareashare_logo.png'),
  background: require('@assets/background.png'),
  logoRefood: require('@assets/Causes/logoRefood.jpg'),
  logoSpeak: require('@assets/Causes/logoSpeak.png'),
  logoGrupoLobo: require('@assets/Causes/logoGrupoLobo.png'),
  logoWorldPressPhoto: require('@assets/Causes/logoWorldPressPhoto.png'),
  logoCantinhoDaMilu: require('@assets/Causes/logoCantinhoDaMilu.png'),
  logoAjudaDeBerco: require('@assets/Causes/logoAjudaDeBerco.png'),
  logoCFAInstitute: require('@assets/Causes/logoCFAInstitute.gif'),
  logoWikipedia: require('@assets/Causes/logoWikipedia.png'),
  piechart: require('@assets/pieChart.png'),
  logoAcademiaCodigo: require('@assets/Causes/logoAcademiaCodigo.png'),
  logoAnimaisRua: require('@assets/Causes/logoAnimaisRua.png'),
  logoBancoAlimentar: require('@assets/Causes/logoBancoAlimentar.jpg'),
  logoCompNacBailado: require('@assets/Causes/logoCompNacBailado.bmp'),
  logoComunidadeVidaPaz: require('@assets/Causes/logoComunidadeVidaPaz.jpg'),
  logoDIANOVA: require('@assets/Causes/logoDIANOVA.jpg'),
  logoEMConservatorioNac: require('@assets/Causes/logoEMConservatorioNac.jpg'),
  logoNationalGeographic: require('@assets/Causes/logoNationalGeographic.png'),
  logoOIKOS: require('@assets/Causes/logoOIKOS.jpg'),
  logoProjectoDelfim: require('@assets/Causes/logoProjectoDelfim.jpg'),
  logoQuercus: require('@assets/Causes/logoQuercus.jpg'),
  logoTNSaoCarlos: require('@assets/Causes/logoTNSaoCarlos.png'),
  logoTheLisbonPlayers: require('@assets/Causes/logoTheLisbonPlayers.png'),
  logoTransparencyInt: require('@assets/Causes/logoTransparencyInt.jpg'),
  logoUnicef: require('@assets/Causes/logoUnicef.png'),
  logoZero: require('@assets/Causes/logoZero.png'),
  logoWorldWildFund: require('@assets/Causes/logoWorldWildFund.png'),
    logoStoik: require('@assets/logoStoik.png'),
};

const fonts = {
  OpenSansBold: require('@assets/Fonts/OpenSans-Bold.ttf'),
  OpenSansItalic: require('@assets/Fonts/OpenSans-Italic.ttf'),
  OpenSansRegular: require('@assets/Fonts/OpenSans-Regular.ttf'),
  RobotoMedium: require('@assets/Fonts/Roboto_Medium.ttf'),
};

const icons = {
  iconSavings: require('@assets/DrawerIcons/iconSavings.png'),
  iconSimulation: require('@assets/DrawerIcons/iconSimulation.png'),
  iconPPR: require('@assets/DrawerIcons/iconPPR.png'),
  iconWallet: require('@assets/DrawerIcons/iconWallet.png'),
  iconPerformances: require('@assets/DrawerIcons/iconPerformances.png'),
  iconShare: require('@assets/DrawerIcons/iconShare.png'),
  iconCauses: require('@assets/DrawerIcons/iconCauses.png'),
  iconBenefits: require('@assets/DrawerIcons/iconBenefits.png'),
  iconOnboarding: require('@assets/DrawerIcons/iconOnboarding.png'),
};

export default { ...images, ...fonts, ...icons };
